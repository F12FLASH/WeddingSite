...
var WeddingServer = class {
  ...
  async gracefulShutdown() {
    this.printMessage("yellow", "\u{1F6D1}", "Received shutdown signal", "Gracefully shutting down server...");
    if (this.server) {
      this.server.close(() => {
  ...
  printBanner() {
    console.log("\n" + "=".repeat(60));
    console.log("\u{1F38A}".padEnd(25) + "WEDDING SITE SERVER".padEnd(25) + "\u{1F38A}");
    console.log("=".repeat(60) + "\n");
  }
  printMessage(color, emoji, title, message = "") {
    const colors2 = {
      green: "\x1B[32m",
      yellow: "\x1B[33m",
      blue: "\x1B[34m",
    ...
    console.log(`${colors2[color]}${emoji}  ${title.padEnd(25)} ${message} ${colors2.reset}${colors2.gray}[${timestamp2}]${colors2.reset}`);
  }
  printError(title, message = "") {
    ...
    console.log(`
${red}\u274C  ${title}${reset}`);
    console.log(`   ${message}${gray} [${timestamp2}]${reset}
`);
  }
  logServerStart(address) {
    const protocol = this.config.env === "development" ? "http" : "https";
    const host = address.address === "::" ? "localhost" : address.address;
    this.printBanner();
    this.printMessage("green", "\u{1F680}", "Server Started Successfully");
    console.log();
    this.printMessage("blue", "\u{1F4CD}", "Server URL", `${protocol}://${host}:${address.port}`);
    this.printMessage("blue", "\u{1F4CA}", "Environment", this.config.env);
    this.printMessage("blue", "\u26A1", "Server Mode", this.config.env === "development" ? "Development (Vite)" : "Production");
    this.printMessage("blue", "\u23F0", "Started at", (/* @__PURE__ */ new Date()).toLocaleString());
    console.log();
    console.log("=".repeat(60));
    console.log();
  }
  async start() {
    try {
      this.validateConfig();
      this.printMessage("blue", "\u{1F527}", "Initializing server", "Wedding Site Server...");
      const app = await createApp();
      this.server = createServer2(app);
...
(async () => {
  try {
    ...
    console.log(`
${colors.red}\u{1F4A5}  Fatal error during server startup:${colors.reset}`);
